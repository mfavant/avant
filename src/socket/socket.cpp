...
std::pair<std::string, int> socket::get_realtime_ip_port() const
{
    if (this->m_sockfd < 0)
    {
        return {};
    ...
    {
        const struct sockaddr_in *s = (const struct sockaddr_in *)&client_addr;
        if (inet_ntop(AF_INET, &s->sin_addr, ip_str, INET6_ADDRSTRLEN) == NULL)
        {
            return {};
        }
        return {ip_str, (int)ntohs(s->sin_port)};
    }
    else if (client_addr.ss_family == AF_INET6)
    {
        const struct sockaddr_in6 *s = (const struct sockaddr_in6 *)&client_addr;
        if (inet_ntop(AF_INET6, &s->sin6_addr, ip_str, INET6_ADDRSTRLEN) == NULL)
        {
            return {};
        }
        return {ip_str, (int)ntohs(s->sin6_port)};
    }
    else
    {
        return {};
